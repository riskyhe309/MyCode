...
public class ModifyKmeans {
    ...
    final double minF2FS = 0.001;  //点与类心之间的最小距离；
    ...
    public ArrayList<ArrayList<String>> cluster(ArrayList<ArrayList<String>> list, int k) {
        ...
        for (int i = 0; i < k; i++) {
            Set<ArrayLabel> tem = new TreeSet<ArrayLabel>();
            asa.add(tem);
        }
        ...
        while (flag && iteration < 100) {
            ...
            asa = allocationLabel(list, k, clusterCenters);
...
//
    ...
    * */
    private void recomputCenter(ArrayList<Set<ArrayLabel>> clusterCenters, ArrayList<Set<ArrayLabel>> al,
        ...
        for (int j = 0; j < clusterCenters.size(); j++) {
            ...
            // b.clear();
            //newClusterCenters.remove(j);
            // if (map.keySet().size() > 0)
            newClusterCenters.add(map.keySet());
        }
    }
    ...
    * 如果最大的f2fs小于阈值，则添加新类别，并将该点加入新的类别
    * */
    private ArrayList<Set<ArrayLabel>> allocationLabel(ArrayList<ArrayList<String>> list, int k,
                                 ArrayList<Set<ArrayLabel>> clusterCenters) {

        /*asa.clear();ArrayList<Set<ArrayLabel>> result*/

        ArrayList<Set<ArrayLabel>> asa = new ArrayList<Set<ArrayLabel>>();

        for (int i = 0; i < clusterCenters.size(); i++){
            Set<ArrayLabel> temp = new HashSet<ArrayLabel>();
            asa.add(temp);
        }
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            for (int j = 0; j < list.get(i).size(); j++) {
                ...
                if (maxFS < minF2FS) {    //与任意一个类心都不匹配，则添加新的类心
                    ...
                    Set<ArrayLabel> a = asa.get(index);
    ...
    * 找出ArrayLabel代表的字符串
    * */
    private String find(ArrayLabel s) {
        return list.get(s.row).get(s.col);
    }
    ...
    * */
    private void randomSelectclusterCenters(ArrayList<ArrayList<String>> features, int K,
        ...
        for (int i = 0; i < K; i++) {
            ...
            int r = (int) (Math.random() * features.get(1).size());
            for (int m = 0; m < K; m++) {
